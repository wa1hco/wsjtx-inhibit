...
- OpenSSL runtime DLLs in the MinGW environment used for Windows
  installer builds

For Windows installer builds, the _OpenSSL_ runtime DLLs bundled with
the installer are taken from the build environment. The DLLs must match
the Qt libraries used by that environment.
