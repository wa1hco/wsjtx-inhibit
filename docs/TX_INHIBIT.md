...
- Inhibit listen port: **always ephemeral** (OS-assigned IPv4). Announced in
  ...
  `host:port` from type 17 — there is no fixed well-known port.
...
2. **PTT port** = real serial device (`COMx` / `/dev/ttyUSBx`), same COM as
   ...
   **before** Enable. Bench without a dongle: Port **`inhibit-sim`**.
...
while TX Inhibit stays enabled. That pulse helps late joiners learn the
**UDP inhibit endpoint**. Counter-only updates (keepalives, invalids)
...
- UDP bind / port discovery failure **fails Inhibit arming**. Digi CAT/PTT
  ...
  from `lineError`. Controllers that reject port 0 (e.g. WIMS) never get a
  usable target from a live type 17.
